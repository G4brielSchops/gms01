...
if knockback = 0 and !place_meeting(x, y, obj_player) and instance_exists(obj_player){
	if point_distance(x, y, obj_player.x, obj_player.y) < 550 and instance_exists(obj_player){
		if point_direction(x, y, obj_player.x, obj_player.y) > 90 and point_direction(x, y, obj_player.x, obj_player.y) < 270{
			left = 1
			right = 0
			if tomar_dano = 0 sprite_index = spr_inimigo_ceu12
		}
		else{
			left = 0
			right = 1
			if tomar_dano = 0 sprite_index = spr_inimigo_ceu1
		}
	}
	else{
		left = 0
...
if obj_player.imune = 0 and place_meeting(x, y, obj_player) and instance_exists(obj_player){
	if left = 1{
		instance_create_layer(x-12, y, layer_create(10, "porrete"), obj_porrete_ceu)
	}
	else{
		instance_create_layer(x+12, y, layer_create(10, "porrete"), obj_porrete_ceu)
	}
